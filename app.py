...
def book_recommend_web(book_name):
    ...
    for i in similar_items:
        ...
        if not temp_df.empty:
            item.extend(list(temp_df.drop_duplicates(
                'BOOK_TITLE')['BOOK_TITLE'].values))
            item.extend(list(temp_df.drop_duplicates(
                'BOOK_TITLE')['BOOK_AUTHOR'].values))
            item.extend(list(temp_df.drop_duplicates(
                'BOOK_TITLE')['IMAGE_URL_M'].values))
        else:
            continue
        data.append(item)
    ...
    df = pd.DataFrame(
        data, columns=["BOOK_TITLE", "BOOK_AUTHOR", "IMAGE_URL_M"])
    return df
...
selected_book_name = st.selectbox(
    "Enter a Book for recommendations:", pt.index)
...
if st.button("Recommend"):
    try:
        ...
        if not recommend_book_df.empty:
            st.subheader("Users who read this book also liked:")
